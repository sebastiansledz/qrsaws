...
interface CreateUserWithRoleData {
  email: string;
  password: string;
  displayName?: string;
  role: 'admin' | 'client' | 'worker';
 ...
 */
export const setUserClaims = functions.region('us-central1').https.onCall(async (data: SetUserClaimsData, ctx: functions.https.CallableContext) => {
  ...
  await admin.firestore().collection('users').doc(uid).set(
    { role, clientId: clientId ?? null, updatedAt: admin.firestore.FieldValue.serverTimestamp() }, 
    { merge: true }
  );
 ...
 */
export const createUserWithRole = functions.region('us-central1').https.onCall(async (data: CreateUserWithRoleData, ctx: functions.https.CallableContext) => {
 ...
 */
async function allocateWZPZSequence(clientId: string, year: number, month: number): Promise<number> {
  ...
  const { seq } = await admin.firestore().runTransaction(async (tx) => {
    ...
    const cur = (snap.exists ? (snap.data()!.seq as number) : 0) + 1;
 ...
 */
export const generateWZPZ = functions.region('us-central1').https.onCall(async (data: GenerateWZPZData, ctx: functions.https.CallableContext) => {
  if (!ctx.auth) throw new functions.https.HttpsError('unauthenticated', '');
 ...
 */
async function generateWZPZInternal(data: GenerateWZPZData, byUserId: string) {
  const { bladeId, type, clientId, code2, movementId, serviceOps } = data;
